...
function valiadte(req, res){
  const JWT = req.headers.authorization.split(' ')[1];
// Split the JWT by `.` to get each part
const jwtHeader = JWT.split('.')[0];
const jwtPayload = JWT.split('.')[1];
const jwtSignature = JWT.split('.')[2];
const decodedHeader = base64.decode(jwtHeader);
const decodedPayload = base64.decode(jwtPayload);
const decodedSignature = base64.decode(jwtSignature);
let expdate=new Date((JSON.parse(decodedPayload)).exp*1000);
//console.log(decodedHeader);
//console.log(decodedPayload);
//console.log(expdate);
let d=Date.now()<expdate;
console.log(d);
return d;
...
ODataServer.prototype.handle = function (req, res) {
  ...
  console.log('in'+JSON.stringify(req.headers))
...
console.log('in'+JSON.stringify(req.headers))
  ...
  function isvalid(req,res){
    let isValidAuth=valiadte(req,res);
